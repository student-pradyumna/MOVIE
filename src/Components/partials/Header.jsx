  ...
  return (
    ...
    >
      ...
      <p className='text-white '>
        <i className=" text-yellow-500 ri-volume-up-fill"></i>{""}{data.release_date || "No Information"}
        <i className="ml-5 text-yellow-500 ri-album-fill"></i>{data.media_type.toUpperCase()}
      </p>
      <Link to={`/${data.media_type}/details/${data.id}/trailer`} className='mt-5 p-4 bg-[#6556CD] rounded text-white '>Watch Trailer..</Link>
    </div>
  )
}
